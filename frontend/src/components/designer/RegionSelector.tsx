  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <select
        ...
        className="h-9 rounded-lg border border-slate-300 bg-white px-3 text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-sky-500"
      >
        {regions.map((r) => (
      ...
      {active && (
        <span className="flex items-center gap-2 text-sm text-slate-500">
          <span>기준 기온 {active.baseAirTemp.toFixed(1)}°C</span>
          <span
            className={cn(
